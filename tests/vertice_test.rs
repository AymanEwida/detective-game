use detective_game::{
    library::utils::convert_angle_to_radians,
    renderer::{
        color::Color,
        styles::Size,
        vertice::{Position, Vertice, _VerticeData},
    },
};

#[test]
fn test_to_position_array() {
    let vertice_position = Position {
        x: 200.0,
        y: -300.0,
    };
...
fn test_get_vertices_data() {
    ...
    let actual = vertice.get_vertice_data();
