use detective_game::{
    game::{
        ...
        level::{GameObject, DEFAULT_SIZE},
        player::{Player, DEFAULT_Q_ABILITY_RADIUS},
        wall::Wall,
    },
    renderer::{styles::Size, vertice::Position},
};
...
fn test_find_optimal_path_same_positions() {
    ...
    let actual = enemy.find_optimal_path(
...
fn test_get_movement_grid_one_obstacles() {
    ...
    let actual =
        enemy.get_movement_grid(input_window_start_position, input_window_size, input_doors);
    let expected = (
        Position { x: 0.0, y: 0.0 },
        vec![
            vec![
...
fn test_enemy_is_off_window_true_on_start_x_axis() {
    ...
    let actual = enemy.is_off_window(Size {
        width: 80.0,
        height: 60.0,
    });
    let expected = true;
...
fn test_enemy_is_off_border_true_on_start_x_axis() {
    ...
    let actual = enemy.is_off_border(
...
fn test_get_near_hide_places_positions_up_to_three_hide_places() {
    ...
    enemy.set_movement_grid(Some(enemy.get_movement_grid(
        Position { x: 0.0, y: 0.0 },
        Size {
            width: 150.0,
            height: 150.0,
    ...
    let input_hide_places_three = vec![
        HidePlace::new(Position { x: 0.0, y: 0.0 }, None),
        HidePlace::new(Position { x: 100.0, y: 20.0 }, None),
