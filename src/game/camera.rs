...
use crate::{
    game::character::DEFAULT_CHARACTER_SIZE,
    library::{
...
use super::{
    character::Direction,
    door::Door,
    enemy::{DetectTraingle, Enemy},
    level::GameObject,
    level_object::ObjectType,
    player::{Player, PlayerStatus},
    wall::Wall,
};

pub const DEFAULT_SIZE_FOR_CAMERA: Size = Size {
    width: 30.0,
    height: 30.0,
};
...
impl Camera<'_> {
    pub fn new_without_repeat(
        position: Position,
        flip: bool,
        scale: Option<f32>,
        rotate: Option<f32>,
    ) -> Self {
        let looking_to = if flip {
            Direction::Right
        } else {
            Direction::Left
        };
    ...
    pub fn new_with_repeat(
        ...
        let interval = if repeat_time <= 0 {
            DEFAULT_REPEAT_INTERVAL
        } else {
            Duration::from_millis(repeat_time)
        };
...
impl<'a> Camera<'a> {
    pub fn draw(&mut self, render: &mut Render<'a>) -> Result<()> {
        if self.is_destroyed {
            render.load_image(
                "assets/game/destroy_camera.png",
                self.position,
                Size {
                    height: 45.0,
                    width: 40.0,
        ...
        if self.is_disturbed {
            assert!(
                ...
                "disturb_duration can not be none"
            );
            ...
            if self.last_updated_time.elapsed() >= self.disturb_duration.unwrap() {
                ...
                render.load_image(
                    "assets/game/disturb_camera.png",
                    self.position,
                    Size {
                        height: 45.0,
                        width: 40.0,
        ...
        if self.repeat && self.last_updated_time.elapsed() >= self.repeat_interval.unwrap() {
            ...
            self.position = Position {
                x: self.position.x + (filp_factor * 12.0),
                y: self.position.y,
            };

            self.set_detect_traingle();

            self.looking_to = if self.flip {
                Direction::Right
            } else {
                Direction::Left
            };
    ...
    pub fn destroy(&mut self, player: &mut Player<'a>) {
    ...
    pub fn set_new_repeat_interval(&mut self, notoriety_level: u64) {
        if let Some(original_repeat_interval) = self.original_repeat_interval {
            if notoriety_level >= 4 {
                if notoriety_level == 4 {
                    self.repeat_interval = Some(Duration::from_millis(
                        original_repeat_interval.as_millis() as u64 - 150,
                    ));
                } else {
                    self.repeat_interval = Some(Duration::from_millis(
                        original_repeat_interval.as_millis() as u64 - 300,
                    ));
                }
            }
        }
    ...
    fn is_detecting_player(
        ...
        if (player.get_status() == &PlayerStatus::Hidden)
            || (self.is_disturbed || self.is_destroyed)
        {
            return false;
        }
        ...
        let camera_start =
            round_position_to_full_numbers(self.position, DEFAULT_MOVEMENT_VALUE, true, true);
        let camera_end = round_position_to_full_numbers(
            camera_start + self.size,
            DEFAULT_MOVEMENT_VALUE,
            true,
            true,
        );

        if simple_object_detect_check(
            player.get_calc_position(),
        ...
        let first_point =
            round_position_to_full_numbers(first_point, DEFAULT_MOVEMENT_VALUE, false, false);
        let second_point =
            round_position_to_full_numbers(second_point, DEFAULT_MOVEMENT_VALUE, false, false);
    ...
    fn get_nearest_enemy(&self, detect_player_position: &Position, enemies: &[Enemy<'a>]) -> isize {
        ...
        let start_position =
            detect_player_position.to_grid_position(*grid_start_position, DEFAULT_MOVEMENT_VALUE);
    ...
    pub fn detect_player(
        &mut self,
        current_notoriety_level: u64,
        player: &mut Player<'a>,
        walls: &[Wall<'a>],
        doors: &[Door<'a>],
        enemies: &[Enemy<'a>],
    ) -> (u64, Option<usize>, Option<Position>) {
