...
use crate::{
    game::{
        ...
        enemy::{EnemyMode, EnemyType, SearchingMode},
        player::{PlayerStatus, ShootObject},
    },
    library::{
        constants::{DEFAULT_MOVEMENT_VALUE, HEIGHT, WIDTH},
        utils::{
            absolute_f32, get_attached_enemy_index, get_correct_start_position,
            get_level_challenges, get_nearest_enemy_id, is_in_circle,
            round_position_to_full_numbers,
        },
    },
    renderer::{
        button::{ButtonAction, OnHoverStylesBuilder},
        color::Color,
        error::Result,
...
use super::{
    ...
    character::Character,
    collectable::{Coin, DoorCollectable, DoorCollectableType},
    detect_range::DetectRange,
    door::{Door, DoorType, ExitDoor, TeleportDoor},
    enemy::Enemy,
    hide_place::HidePlace,
    player::{InventoryItem, Player, DEFAULT_SIZE_FOR_INVENTORY_ITEM},
    store::StoreItem,
    wall::Wall,
};

pub const DEFAULT_SIZE: f32 = 30.0;
pub const DEFAULT_SIZE_FOR_HIDE_PLACE: Size = Size {
    width: 45.0,
    height: 65.0,
};
pub const DEFAULT_SIZE_FOR_COLLECTABLE: Size = Size {
    width: 40.0,
    height: 40.0,
};
...
pub fn display_holding_item<'a>(
    start_position: Position,
    holding_item: Option<InventoryItem<'a>>,
    scale: f32,
    render: &mut Render<'a>,
) -> Result<()> {
    ...
    if let Some(item) = holding_item {
        ...
        render.load_image(
            item.get_image(),
            Position {
                x: start_position.x + 100.0,
                y: start_position.y + (50.0 - (negate_scale * 50.0)),
            },
        ...
        render.display_text(
            &format!("| {}", item.get_amount()),
            Position {
                x: start_position.x + 150.0,
                y: start_position.y + 50.0,
            },
            scale,
            None,
            Color::White,
        ...
        if let Some(ammo) = item.get_ammo() {
            ...
            render.load_image(
                item.get_ammo_image().unwrap(),
                Position {
                    x: start_position.x + 290.0,
                    y: start_position.y + (50.0 - (negate_scale * 45.0)),
                },
                Size {
                    width: 50.0,
                    height: 50.0,
...
pub fn display_tutorial<'a>(
    ...
    render.display_text_with_images(
        tutorial_text,
        start_position,
        end_x,
        Size {
...
impl<'a> GameLevel<'a> {
    pub fn get_boder_size(&self) -> Size {
        Size {
            width: self.border_size.width - (DEFAULT_SIZE * 2.0),
            height: self.border_size.height - (DEFAULT_SIZE * 2.0),
        }
    }
    ...
    pub fn draw(
        &mut self,
        player: &mut Player<'a>,
        store_items: &mut [StoreItem<'a>],
        store_items_len: usize,
        render: &mut Render<'a>,
    ) -> Result<()> {
        if self.get_status() == &LevelStatus::ReLoadLevel {
            self.load_level(player)
                .expect(&format!("Can not load level: {}", self.current_level));
        } else if self.get_status() == &LevelStatus::Lose {
            render.display_text(
                "You Lost!",
                Position {
                    x: (WIDTH as f32 / 2.0) - 135.0,
                    y: (HEIGHT as f32 / 4.0) + 30.0,
                },
                1.5,
                None,
                Color::Red,
            ...
            for (idx, challenge) in self.challenges.iter().enumerate() {
                ...
                render.display_text(
                    &format!(
                        "Challenge {}: {} - completed +{} coins",
                        idx + 1,
                        challenge.get_challenge_text(),
                        challenge.get_reward()
                    ),
                    Position {
                        x: 550.0,
                        y: 180.0 + (idx as f32 * 40.0),
                    },
                    0.6,
                    None,
                    color,
            ...
            render.load_image(
                ...
                Position { x: 900.0, y: 370.0 },
                DEFAULT_SIZE_FOR_COLLECTABLE,
                false,
                None,
                None,
                None,
            ...
            render.display_text(
                &format!("{}", player.get_coins()),
                Position { x: 940.0, y: 370.0 },
                1.0,
                None,
                Color::White,
            )?;
            ...
            for idx in *self.start_idx_store_items.borrow()..*self.end_idx_store_items.borrow() {
                ...
                render.load_image(
                    store_item.get_image_path(),
                    Position {
                        x: 185.0 + (idx as f32 * offset),
                        y: 440.0,
                    },
                    Size {
                        width: 480.0,
                ...
                let (title_size, _) = render.display_text(
                    store_item.get_title(),
                    Position {
                        x: 275.0 + (idx as f32 * offset),
                        y: 645.0,
                    },
                    0.6,
                    Some(370.0),
                ...
                render.display_text(
                    store_item.get_description(),
                    Position {
                        x: 185.0 + (idx as f32 * offset),
                        y: 645.0 + title_size.height + 10.0,
                    },
                    0.5,
                    Some(490.0),
                    Color::Black,
                ...
                render.display_button(ButtonProps {
                    ...
                    click_action: ButtonAction::BuyStoreItem(
                        idx + *self.start_idx_store_items.borrow(),
                    ),
                    on_click: Box::new(|| {}),
                    on_hover: Box::new(|| {}),
                    on_hover_release: Box::new(|| {}),
                ...
                if let Some(upgrade_info) = store_item.get_upgrade_info() {
                    render.display_text(
                        &format!("({}/{})", upgrade_info.0, upgrade_info.1),
                        Position {
                            x: 275.0 + (idx as f32 * offset),
                            y: 875.0,
                        },
                ...
                render.display_text(
                    &format!("{}", store_item.get_price()),
                    Position {
                        x: 625.0 + (idx as f32 * offset),
                        y: 890.0,
                    },
                    0.6,
                    None,
            ...
            for num in 0..8 {
                ...
                render.load_image(
                    ...
                    Position {
                        x: self.border_top_left.x + num as f32 * (self.border_size.width / 8.0),
                        y: self.border_top_left.y,
                    },
                    Size {
                        width: self.border_size.width / 8.0,
                        height: DEFAULT_SIZE,
                    },
                    false,
                    None,
                ...
                render.load_image(
                    ...
                    Position {
                        x: self.border_top_left.x + num as f32 * (self.border_size.width / 8.0),
                        y: self.border_top_left.y + self.border_size.height - DEFAULT_SIZE,
                    },
                    Size {
                        width: self.border_size.width / 8.0,
                        height: DEFAULT_SIZE,
            ...
            for num in 0..2 {
                ...
                render.load_image(
                    "assets/game/wall.jpg",
                    Position {
                        x: self.border_top_left.x + self.border_size.width - DEFAULT_SIZE,
                        y: self.border_top_left.y
                            + ((num as f32 - 1.0) * DEFAULT_SIZE).abs()
                            + num as f32 * (self.border_size.height / 2.0),
                    },
                    Size {
                        width: DEFAULT_SIZE,
                        height: (self.border_size.height - 60.0) / 2.0,
                    },
                    false,
                    None,
                    None,
            ...
            if !(*self.is_in_tutorial.borrow()) {
                for (idx, challenge) in self.challenges.iter_mut().enumerate() {
                    if challenge.get_status() == &ChallengeStatus::NotDetermine {
                        challenge.set_status(challenge.check_challenge(
                            player,
                            false,
                    ...
                    let color = match challenge.get_status() {
                        &ChallengeStatus::Completed => Color::Green,

                        &ChallengeStatus::Failed => Color::Red,

                        &ChallengeStatus::NotDetermine => Color::White,
                    };
            ...
            render.display_text(
                &format!("level: {}", self.current_level),
                Position { x: 1580.0, y: 50.0 },
                0.6,
                None,
                Color::White,
            )?;
            ...
            render.display_text(
                &format!("lifes: {}", player.get_lifes()),
                Position { x: 920.0, y: 900.0 },
                0.8,
                None,
                Color::White,
            )?;
            ...
            for door in self.doors.iter_mut() {
                let collided_enemies: Vec<&Enemy<'a>> = self
                    .enemies
                    .iter()
                    .filter(|enemy| !enemy.get_is_dead() && enemy.collide(door))
                    .collect();
            ...
            for teleport_door in self.teleport_doors.iter() {
                let want_to_teleport_enemies: Vec<&mut Enemy<'a>> = self
                    .enemies
                    .iter_mut()
                    .filter(|enemy| {
                        ...
                        !enemy.get_is_teleported()
                            && enemy.get_want_to_teleport_id().unwrap() == teleport_door.get_id()
                            && enemy.collide(teleport_door)
                    })
                    .collect();
                    ...
                    for enemy in want_to_teleport_enemies {
                        ...
                        if enemy.get_mode() != &EnemyMode::Regular {
                            if enemy.get_should_attach_teleport_door() {
                                enemy.attach_teleport_door(
                                    teleport_door.get_id(),
                                    teleport_door.get_move_to_id(),
                ...
                if player.is_colliding_with_object(teleport_door) {
                    if let Some(player_interaction) = player.get_interaction() {
                        if !player.get_is_teleported()
                            && player_interaction.key() == &Key::Space
                            && player_interaction.action() == &Action::Press
                        {
            ...
            for door_collectable in self.door_collectables.iter_mut() {
                if !door_collectable.is_collected() && player.collide(door_collectable) {
                    player.add_door_collectable(
                        door_collectable.get_door_collectable_type(),
                        door_collectable.get_id(),
                        door_collectable.opens(),
                    );
            ...
            if player.get_status() == &PlayerStatus::NotHidden && player.collide(exit_door) {
                if let Some(player_interaction) = player.get_interaction() {
                    if player_interaction.key() == &Key::Space
                        && player_interaction.action() == &Action::Press
                    {
                ...
                if player.is_colliding_with_object(hide_place) {
                    if let Some(player_interaction) = player.get_interaction() {
                        ...
                        if (!player.get_is_detected_by_enemy()
                            || (player.get_is_detected_by_enemy()
                                && !player.get_is_seen_by_enemy())
                            || player_status != &PlayerStatus::Detectit)
                            && player_interaction.key() == &Key::Space
                            && player_interaction.action() == &Action::Press
                        {
            ...
            for camera in self.cameras.iter_mut() {
                ...
                let (new_notoriety_level, enemy_id, detected_player_position) = camera
                    .detect_player(
                        self.notoriety_level,
                        player,
                        &self.walls,
                ...
                if let Some(enemy_id) = enemy_id {
                    self.attached_enemies_ids.push((
                        enemy_id,
                        detected_player_position.unwrap(),
            ...
            for _ in 0..self.detecting_ranges.len() {
                ...
                for enemy in &self.enemies {
                    if !enemy.get_is_dead()
                        && !enemy.get_is_searching_detect_area()
                        && detect_range.is_in_range(enemy)
                    {
                ...
                if enemies_in_detect_area.len() > 0 {
                    let mut start_position = round_position_to_full_numbers(
                        detect_range.get_center_position(),
                    ...
                    start_position = get_correct_start_position(
                        start_position,
                        movement_grid,
                        enemy.get_movement_value(),
                    );

                    let nearest_enemy_id =
                        get_nearest_enemy_id(start_position, &enemies_in_detect_area);

                    if nearest_enemy_id != -1 {
                        self.attached_enemies_ids.push((
                            nearest_enemy_id as usize,
                            start_position,
            ...
            for enemy in self.enemies.iter_mut() {
                ...
                if idx != -1 {
                    let (.., attached_position, attached_type) =
                        &self.attached_enemies_ids[idx as usize];
                ...
                if enemy.collide_with_player(&player)
                    && (self.status != LevelStatus::ReLoadLevel || self.status != LevelStatus::Lose)
                {
            ...
            for _ in 0..self.bullets.size() {
                ...
                if !bullet.get_is_finished() {
                    let mut is_object_colliding = bullet.is_off_border(
                        Some(self.get_boder_start_position()),
                        self.get_boder_size(),
                    );
                    ...
                    if !is_object_colliding {
                        for camera in self.cameras.iter_mut() {
                            if bullet.collide_with_camera(camera) {
                                if bullet.get_bullet_type() == &BulletType::CameraGunBullet
                                    && !camera.get_is_disturbed()
                                {
                                    camera.set_is_disturbed(
                                        true,
                                        Some(player.get_notoriety_camera_disturb_lifttime()),
                                    );
                                    player.add_to_disturb_cameras_count(1);
                                } else if bullet.get_bullet_type()
            ...
            for _ in 0..self.cans.size() {
                ...
                if !can.get_is_finished() {
                    let mut is_object_colliding = can.is_off_border(
                        Some(self.get_boder_start_position()),
                        self.get_boder_size(),
                    );
                ...
                if !can.get_is_finished() {
                    ...
                    if !can.get_added_detect_range() {
                        self.detecting_ranges.push(DetectRange::new(
                            player.get_can_detecting_radius(),
                            can.get_calc_position().0,
                        ));
            ...
            if *self.is_in_tutorial.borrow() {
                ...
                render.draw_rectangle(
                    Position {
                        x: player.get_position().x - 75.0 + offset_x,
                        y: player.get_position().y - 160.0,
                    },
                    Size {
                        width: 300.0,
                        height: 150.0,
                ...
                if *self.tutorial_idx.borrow() != 0 {
                    render.display_button(ButtonProps {
                        position: Position {
                            x: player.get_position().x - 60.0 + offset_x,
                            y: player.get_position().y - 40.0,
                        },
                        width: None,
                        height: None,
                        padding: Padding::new(10.0, 10.0, 20.0, 20.0),
                ...
                render.display_button(ButtonProps {
                    position: Position {
                        x: player.get_position().x + 155.0 + offset_x,
                        y: player.get_position().y - 40.0,
                    },
                    width: None,
                    height: None,
                    padding: Padding::new(10.0, 10.0, 20.0, 20.0),
                    bg_color: Color::RGB(169, 169, 169),
                    text: if *self.tutorial_idx.borrow()
                        < (self.tutorials[self.current_level as usize].len() - 1)
                    {
                        String::from("Next")
                    } else {
                        String::from("Finish")
                ...
                display_tutorial(
                    &self.tutorials,
                    Position {
                        x: player.get_position().x - 70.0 + offset_x,
                        y: player.get_position().y - 155.0,
                    },
                    player.get_position().x + 220.0 + offset_x,
                    self.current_level,
                    *self.tutorial_idx.borrow(),
                    render,
                )?;
            }
        }
    ...
    pub fn next_level(&mut self) {
        assert!(
            ...
            "level must be between 0 to 5 (include)"
        );
    ...
    pub fn clear_level_objects(&mut self) {
        if self.status != LevelStatus::Lose && self.current_level != 0 {
            self.challenges =
                get_level_challenges(self.current_level).expect("Unable to get level challenges");
        }
    ...
    pub fn load_level(&mut self, player: &mut Player<'a>) -> std::result::Result<(), String> {
        ...
        match self.current_level {
            0 => {
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 200.0, y: 360.0 },
                    player.get_enemy_detect_range(),
                    "19l/0 17u/3000 19r/0 17d/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 200.0, y: 620.0 },
                    player.get_enemy_detect_range(),
                    "17u/4500 19l/0 17d/0 19r/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 290.0, y: 620.0 },
                    player.get_enemy_detect_range(),
                    "8r/0 16u/3000 16d/0 8l/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 450.0, y: 200.0 },
                    player.get_enemy_detect_range(),
                    "8l/0 15d/3000 15u/0 8r/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "18l/0 10d/0 35r/3500 35l/0 10u/0 18r/5000",
                    false,
                ));

                self.insert_wall(Wall::new(
                ...
                self.insert_door(Door::new(
                    0,
                    DoorType::Regular,
                    Position { x: 245.0, y: 620.0 },
                    Size {
                        width: DEFAULT_SIZE + 5.0,
                        height: 60.0,
                    },
                    false,
                    None,
            ...
            1 => {
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 140.0, y: 10.0 },
                    player.get_enemy_detect_range(),
                    "18d/0 13l/3000 13r/0 18u/0 6r/3000 6l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 100.0, y: 295.0 },
                    player.get_enemy_detect_range(),
                    "9l/6000 20r/3000 11l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 10.0, y: 405.0 },
                    player.get_enemy_detect_range(),
                    "11d/0 15r/0 10d/0 5l/2000 9r/2000 21u/1000 19l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 380.0, y: 420.0 },
                    player.get_enemy_detect_range(),
                    "25r/0 6u/2000 16d/2000 25l/0 9d/0 5l/2000 5r/0 19u/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 550.0, y: 257.0 },
                    player.get_enemy_detect_range(),
                    "8r/5000 15l/3000 7r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 470.0, y: 157.0 },
                    player.get_enemy_detect_range(),
                    "16r/4000 16l/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 470.0, y: 5.0 },
                    player.get_enemy_detect_range(),
                    "16r/3000 5d/0 16l/0 5u/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "18u/3000 4r/0 18d/0 4l/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 915.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "15d/2000 4l/0 15u/0 15l/0 15d/2000 4r/0 15u/0 15r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 865.0, y: 335.0 },
                    player.get_enemy_detect_range(),
                    "18d/3000 4r/0 18u/5000 4l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "42u/6000 42d/0 4l/4500 4r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 730.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "20r/6000 20l/4000",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "18u/6000 18d/0 2l/4500 2r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1310.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "20d/6000 20u/0 2r/5500 2l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1450.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "24d/6000 24u/0 2l/4500 2r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "18u/5500 18d/0 2r/6000 2l/2500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "12r/0 1d/4000 42u/3000 12l/0 41d/0",
                    false,
                ));
                ...
                self.insert_door_collectable(DoorCollectable::new(
                    1,
                    DoorCollectableType::CodePaper,
                    Position { x: 5.0, y: 625.0 },
                    vec![1],
                    None,
                ));

                self.insert_wall(Wall::new(
                    Position { x: 505.0, y: 580.0 },
                    Size {
                        width: DEFAULT_SIZE,
                        height: 96.0,
                    },
                ...
                self.insert_hide_place(HidePlace::new(
                    Position {
                        x: 1065.0,
                        y: 130.0,
                    },
                ...
                self.insert_exit_door(ExitDoor::new(
                    Position {
                        x: 1697.0,
                        y: 620.0,
                    },
            ...
            2 => {
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 960.0, y: 620.0 },
                    player.get_enemy_detect_range(),
                    "11u/0 21r/5500 21l/0 10d/0 1r/6000 1l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "11l/7000 2r/1000 17d/4000 9r/0 17u/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "10r/4500 8d/6500 10l/0 8u/2500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 840.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "19u/0 2r/6000 2l/0 19d/0 2l/5500 2r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 10.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "35r/5000 35l/5000",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "4r/4000 4l/0 23d/6000 23u/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 205.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "5l/4500 14d/0 5r/5500 14u/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 550.0, y: 330.0 },
                    player.get_enemy_detect_range(),
                    "30r/4500 30l/5500",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 475.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "6r/0 19u/4000 16r/0 19d/0 2r/3500 24l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 5.0, y: 345.0 },
                    player.get_enemy_detect_range(),
                    "36r/2500 7d/0 36l/0 7u/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 335.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "29d/0 12r/0 4u/3500 4d/0 12l/0 29u/0 2r/3500 2l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 475.0, y: 150.0 },
                    player.get_enemy_detect_range(),
                    "38r/3500 38l/5500",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 475.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "3d/0 73r/7500 73l/0 3u/0 1l/6500 1r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "1r/0 27d/0 1r/4000 1l/0 27u/0 1l/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "26d/0 3l/6000 3r/0 26u/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1360.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "17r/6500 6d/5500 17l/0 6u/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "8l/0 18d/0 8r/0 1d/6500 19u/4500",
                    false,
                ));
                ...
                self.insert_camera(Camera::new_without_repeat(
                    Position {
                        x: 1450.0,
                        y: 582.0,
                    },
                ...
                self.insert_camera(Camera::new_with_repeat(
                    Position { x: 985.0, y: 395.0 },
                    false,
                    None,
                    None,
                    Some(6000),
                ...
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 795.0, y: 550.0 },
                    true,
                    None,
                    Some(20.0),
                ));

                self.insert_teleport_door(TeleportDoor::new(
                    1,
                    Position { x: 475.0, y: 615.0 },
                    Position { x: 560.0, y: 790.0 },
                    1,
                    None,
                    None,
                ));
                ...
                self.insert_teleport_door(TeleportDoor::new(
                    3,
                    Position { x: 5.0, y: 335.0 },
                    Position { x: 90.0, y: 510.0 },
                    3,
                    None,
                    None,
                ));
                ...
                self.insert_door(Door::new(
                    2,
                    DoorType::Locked,
                    Position { x: 417.0, y: 0.0 },
                    Size {
                        width: DEFAULT_SIZE + 15.0,
                        height: 60.0,
                    },
                    true,
                    Some(2),
                ...
                self.insert_teleport_door(TeleportDoor::new(
                    5,
                    Position { x: 545.0, y: 420.0 },
                    Position { x: 620.0, y: 600.0 },
                    5,
                    None,
                    None,
                ));
                ...
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 607.5, y: 95.0 },
                    true,
                    None,
                    None,
                ));
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 858.5, y: 95.0 },
                    true,
                    None,
                    None,
                ));
                ...
                self.insert_door(Door::new(
                    5,
                    DoorType::Coded,
                    Position { x: 1580.0, y: 0.0 },
                    Size {
                        width: DEFAULT_SIZE + 18.0,
                        height: 60.0,
                    },
                    true,
                    Some(1),
                ...
                self.insert_wall(Wall::new(
                    Position { x: 1677.0, y: 63.0 },
                    Size {
                        width: DEFAULT_SIZE - 10.0,
                        height: 117.0,
                    },
                    None,
                    None,
            ...
            3 => {
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1525.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "13d/0 15r/0 1d/3500 1u/0 15l/0 13u/0 5r/6000 5l/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 342.0, y: 130.0 },
                    player.get_enemy_detect_range(),
                    "33l/0 3u/3000 3d/0 33r/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 950.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "3r/0 13d/0 2r/6500 2l/0 13u/0 3l/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1145.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "3l/0 13d/0 2l/6000 2r/0 13u/0 3r/5000",
                    true,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "18r/0 13d/0 8l/4000 8r/0 13u/0 18l/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "1r/0 31d/0 1r/4500 1l/0 30u/0 1l/6500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "1l/0 31d/0 1l/4500 1r/0 31u/0 1r/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "10r/3500 20d/5500 10l/0 20u/0",
                    true,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 970.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "20r/4500 20l/6500",
                    true,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 970.0, y: 220.0 },
                    player.get_enemy_detect_range(),
                    "29d/4500 29u/0 1l/6000 1r/0",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 835.0, y: 310.0 },
                    player.get_enemy_detect_range(),
                    "2r/0 20d/7000 20u/0 2l/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 729.0, y: 310.0 },
                    player.get_enemy_detect_range(),
                    "2l/0 20d/5500 20u/0 2r/5000",
                    true,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 604.0, y: 320.0 },
                    player.get_enemy_detect_range(),
                    "20d/0 2l/4000 2r/0 20u/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 421.0, y: 520.0 },
                    player.get_enemy_detect_range(),
                    "6r/4500 19u/3000 5l/2000 19d/0 1l/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 330.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "35r/4000 35l/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 221.0, y: 310.0 },
                    player.get_enemy_detect_range(),
                    "9r/0 11d/3500 8l/0 11u/0 1l/5500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 110.0, y: 310.0 },
                    player.get_enemy_detect_range(),
                    "10l/0 20d/5500 9r/0 20u/0 1r/3500",
                    false,
                ));

                self.insert_wall(Wall::new(
                    Position { x: 1639.0, y: 60.0 },
                    Size {
                        width: 120.0,
                        height: DEFAULT_SIZE,
                    },
                ...
                self.insert_teleport_door(TeleportDoor::new(
                    10,
                    Position { x: 720.0, y: 215.0 },
                    Position { x: 90.0, y: 390.0 },
                    12,
                    None,
                    None,
                ));
                ...
                self.insert_teleport_door(TeleportDoor::new(
                    12,
                    Position { x: 10.0, y: 215.0 },
                    Position { x: 800.0, y: 390.0 },
                    10,
                    None,
                    None,
                ));
                ...
                self.insert_door(Door::new(
                    3,
                    DoorType::Coded,
                    Position { x: 363.0, y: 520.0 },
                    Size {
                        width: DEFAULT_SIZE + 30.0,
                        height: 60.0,
                    },
                    true,
                    Some(2),
            ...
            4 => {
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    Position { x: 0.0, y: 420.0 },
                    player.get_enemy_detect_range(),
                    "10r/0 10u/4000 10l/0 10d/6000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 0.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "29r/0 13d/0 7l/3000 13u/0 12l/0 12d/3000 10l/0 12u/3000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 285.0, y: 430.0 },
                    player.get_enemy_detect_range(),
                    "9l/0 11u/5500 8r/0 11d/0 1r/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 704.0, y: 301.0 },
                    player.get_enemy_detect_range(),
                    "27l/0 13d/0 4l/6500 8r/0 13u/0 23r/4500",
                    true,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 584.0, y: 191.0 },
                    player.get_enemy_detect_range(),
                    "17l/3500 10u/0 29r/3500 29l/0 10d/0 17r/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 842.0, y: 411.0 },
                    player.get_enemy_detect_range(),
                    "11d/0 13l/5000 9u/0 11l/3000 24r/0 2u/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "5d/0 5l/0 17d/5500 17u/0 5r/0 5u/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1685.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "32l/3500 32r/5500",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 812.5, y: 100.0 },
                    player.get_enemy_detect_range(),
                    "1d/0 3r/0 8d/3500 20r/4000 9u/0 23l/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1460.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "5r/0 33d/6000 33u/0 5l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1355.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "10d/3000 21l/2500 7u/3000 4r/0 3u/0 17r/5000",
                    true,
                ));
                ...
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 227.0, y: 585.0 },
                    true,
                    None,
                    None,
                ));
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 455.0, y: 585.0 },
                    true,
                    None,
                    None,
                ));
                ...
                self.insert_wall(Wall::new(
                    Position { x: 575.0, y: 361.0 },
                    Size {
                        width: 197.5,
                        height: DEFAULT_SIZE + 10.0,
                    },
                    None,
                    None,
                ));
                ...
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 305.0, y: -25.0 },
                    true,
                    None,
                    None,
                ));
                self.insert_camera(Camera::new_with_repeat(
                    Position { x: 522.5, y: -25.0 },
                    false,
                    None,
                    None,
                    Some(4000),
                ...
                self.insert_teleport_door(TeleportDoor::new(
                    1,
                    Position { x: 704.5, y: 186.0 },
                    Position { x: 670.0, y: 690.0 },
                    2,
                    None,
                    None,
                ));
            ...
            5 => {
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 883.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "3l/0 21u/5500 21d/0 3r/3500",
                    true,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    ...
                    player.get_enemy_detect_range(),
                    "2r/0 12u/0 19r/2000 11d/4000 11u/0 19l/0 12d/0 2l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 977.0, y: 510.0 },
                    player.get_enemy_detect_range(),
                    "1d/0 40r/6500 40l 1u/4500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 988.0, y: 301.0 },
                    player.get_enemy_detect_range(),
                    "11r/0 14u/0 11l/0 7u/2000 11r/4000 11l/0 7d/0 11r/0 14d/0 11l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "11r/2000 6d/0 11l/0 14d/0 11r/3000 11l/0 14u/0 11r/0 6u/0 11l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "13r/0 10u/0 11l/2500 11u/0 11r/2500 21d/0 1r/3500 14l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "2r/0 19u/0 3r/0 2u/4000 2d/0 3l/0 19d/0 2l/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 1364.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "32r/5500 32l/4500",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 155.0, y: 0.0 },
                    player.get_enemy_detect_range(),
                    "3r/0 6d/0 14r/0 6u/0 6r/4000 6l/0 6d/0 14l/0 6u/0 3l/4000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 698.0, y: 291.0 },
                    player.get_enemy_detect_range(),
                    "2l/0 22d/4000 22u/0 2r/5000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 488.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "20r/5000 20l/7000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 320.0, y: 615.0 },
                    player.get_enemy_detect_range(),
                    "2r/0 16u/0 22r/2000 7d/0 8l/3500 7u/0 14l/0 16d/0 2l/5500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 0.0, y: 380.0 },
                    player.get_enemy_detect_range(),
                    "13d/3500 5r/0 1d/0 1r/3500 13u/0 6l/0 1u/7000",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 160.0, y: 520.0 },
                    player.get_enemy_detect_range(),
                    "3r/0 15u/0 24r/3500 24l/0 15d/0 3l/5500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 815.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "3r/0 9d/7500 9u/0 3l/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 705.0, y: 90.0 },
                    player.get_enemy_detect_range(),
                    "21l/5000 21r/6500",
                    false,
                ));
                ...
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 323.0, y: 150.0 },
                    player.get_enemy_detect_range(),
                    "1r/0 3d/0 5r/3500 2l/0 10d/0 2r/3000 5l/2500 13u/0 1l/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 162.0, y: 280.0 },
                    player.get_enemy_detect_range(),
                    "3r/0 13u/0 3r/5500 3l/0 13d/0 3l/3500",
                    false,
                ));
                self.insert_enemy(Enemy::new(
                    EnemyType::Regular,
                    ...
                    player.get_enemy_detect_range(),
                    "17d/3500 5r/0 1d/0 1r/3500 1l/0 15u/0 5l/0 3u/3500",
                    false,
                ));
                ...
                self.insert_door_collectable(DoorCollectable::new(
                    2,
                    DoorCollectableType::CodePaper,
                    Position { x: 582.5, y: 460.0 },
                    vec![2],
                    None,
                ));

                self.insert_wall(Wall::new(
                    Position { x: 282.5, y: 430.0 },
                    Size {
                        width: 340.0,
                        height: DEFAULT_SIZE,
                    },
                ...
                self.insert_teleport_door(TeleportDoor::new(
                    3,
                    Position { x: 532.5, y: 365.0 },
                    Position { x: 140.0, y: 450.0 },
                    4,
                    None,
                    None,
                ));
                ...
                self.insert_door_collectable(DoorCollectable::new(
                    3,
                    DoorCollectableType::Key,
                    Position { x: 722.5, y: 201.0 },
                    vec![3],
                    None,
                ));
                self.insert_camera(Camera::new_without_repeat(
                    Position { x: 672.5, y: 155.0 },
                    true,
                    None,
                    None,
                ));
