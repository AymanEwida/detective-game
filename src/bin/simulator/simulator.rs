use detective_game::{
    game::{
        ...
        can::Can,
        character::Character,
        collectable::{DoorCollectable, DoorCollectableType},
        detect_range::DetectRange,
        door::{Door, DoorType, TeleportDoor},
        enemy::{Enemy, EnemyMode, EnemyType, SearchingMode},
        hide_place::HidePlace,
        level::{display_holding_item, AttachedType, GameObject, DEFAULT_SIZE},
        player::{Player, PlayerStatus, ShootObject, DEFAULT_Q_ABILITY_RADIUS},
        wall::Wall,
    },
    library::{
        constants::DEFAULT_MOVEMENT_VALUE,
        utils::{
            get_attached_enemy_index, get_correct_start_position, get_nearest_enemy_id,
            is_in_circle, round_position_to_full_numbers,
        },
    },
...
impl std::fmt::Display for SimulationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            Self::LoadSimulationError(simulation_type, message) => write!(
                f,
                "Load Simulation Error: {}\nin: {:?}",
                message, simulation_type
            ),
        }
    }
}
...
impl<'a> Simulator<'a> {
    pub fn draw(&mut self, player: &mut Player<'a>, render: &mut Render<'a>) -> Result<()> {
            ...
            if player.is_colliding_with_object(hide_place) {
                if let Some(player_interaction) = player.get_interaction() {
                    ...
                    if (!player.get_is_detected_by_enemy()
                        || (player.get_is_detected_by_enemy() && !player.get_is_seen_by_enemy())
        ...
        for camera in self.cameras.iter_mut() {
            ...
            let (new_notoriety_level, enemy_id, detected_player_position) = camera.detect_player(
        ...
        match simulator_type {
            SimulatorType::EnemyLogic => {
                self.enemies.push(Enemy::new(
                    EnemyType::Regular,
                    Position { x: 290.0, y: 260.0 },
                    DEFAULT_Q_ABILITY_RADIUS,
                    "6u/5500 6r/0 6d/5500 6u/0 9r/5500 6d/5500 15l/5500",
                    false,
                ));

                self.walls.push(Wall::new(
                    Position { x: 250.0, y: 170.0 },
                    Size {
                        width: 250.0,
                        height: DEFAULT_SIZE,
                    },
                ...
                self.doors.push(
                    ...
                    .map_err(|error| {
                        SimulationError::LoadSimulationError(
                            simulator_type.clone(),
                            error.to_string(),
                        )
                    })?,
                );
                self.walls.push(Wall::new(
                    Position { x: 500.0, y: 170.0 },
                    Size {
                        width: DEFAULT_SIZE,
                        height: 180.0,
                    },
                ...
                self.hide_places
                    .push(HidePlace::new(Position { x: 302.0, y: 255.0 }, None));
                self.hide_places
                    .push(HidePlace::new(Position { x: 375.0, y: 200.0 }, None));
                self.hide_places
                    .push(HidePlace::new(Position { x: 455.0, y: 240.0 }, None));
            }

            SimulatorType::EnemySearchLogic => {
                // self.enemies.push(Enemy::new(EnemyType::Regular, Position { x: 290.0, y: 260.0 }, DEFAULT_Q_ABILITY_RADIUS, "6u/5500 6r/0 6d/5500 6u/0 9r/5500 6d/5500 15l/5500", false));
                self.enemies.push(Enemy::new(
                    EnemyType::Regular,
                ...
                self.hide_places
                    .push(HidePlace::new(Position { x: 100.0, y: 60.0 }, None));
                self.teleport_doors.push(TeleportDoor::new(
                    1,
                    Position { x: 200.0, y: 80.0 },
            ...
            SimulatorType::PlayerInteractionWithHidePlace => {
                self.hide_places
                    .push(HidePlace::new(Position { x: 300.0, y: 400.0 }, None));
            }
            ...
            SimulatorType::CameraLogic => {
                ...
                // self.enemies.push(Enemy::new(EnemyType::Regular, Position { x: 500.0, y: 20.0 }, DEFAULT_Q_ABILITY_RADIUS, "4r/2500 4l/2500", false));
                ...
                self.doors.push(
                    ...
                    .map_err(|error| {
                        SimulationError::LoadSimulationError(simulator_type, error.to_string())
                    })?,
                );
                ...
                self.cameras.push(Camera::new_with_repeat(
                    Position { x: 370.0, y: 175.0 },
                    false,
                    None,
                    None,
                    Some(5000),
            ...
            SimulatorType::DoorLogic => {
                ...
                self.door_collectables.push(DoorCollectable::new(
                    1,
                    DoorCollectableType::Key,
                    Position { x: 400.0, y: 20.0 },
                    vec![1],
                    None,
                ));
                self.door_collectables.push(DoorCollectable::new(
